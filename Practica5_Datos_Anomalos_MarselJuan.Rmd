...
Los valores de sepal.length mínimo es 0 lo cuál es inconsistente
El valor mínimo de Sepal.Width es negativo y es imposible porque es un tipo de dato de razón
El valor mínimo de Petal.Length es 0 que también es imposible

El valor máximo de Petal.Length es 63 que es mucho más grande que el resto de datos (outlier)
Los valores máximos de Sepal.Width también llaman la atención y sobresalen del resto
El valor de Sepal.Length también sorbe sale del máximo.
Hay un dato inf en petal.Width

Los conjuntos de Species están bien
...
df_na<-dirty_iris %>% mutate(across(where(is.numeric),~if_else(is.infinite(.) | .<=0,NA,.)))
...
num_completas <- sum(complete.cases(df_na))
total_filas <- nrow(df_na)
proporcion_completa <- num_completas / total_filas
porcentaje_completa <- proporcion_completa * 100
porcentaje_completa
clean_iris <- na.omit(df_na)
...
```{r , include = TRUE}
clean_iris %>% summarise(across(.cols=(Petal.Length),.fns=list(mean=~mean(.x),sd=~sd(.x),mediana=~median(.x),MADM=~1.4826*median(abs(.x-median(.x))),IQR=~IQR(.x))))

df_na %>% summarise(across(.cols=(Petal.Length),.fns=list(mean=~mean(.x,na.rm=T),sd=~sd(.x,na.rm=T),mediana=~median(.x,na.rm=T),MADM=~1.4826*median(abs(.x-median(.x,na.rm=T)),,na.rm=T),IQR=~IQR(.x,na.rm=T))))
```
Vemos como la mediana y el MADM no cambian entre los dos conjuntos, el IQR cambia muy poco y la media y la desviación cambian mucho porque estas métricas son muy sensibles a valores faltantes y outliers.
...
reglasigma<-function(x){
  ...
  m<-mean(x,na.rm=T)
  s<- sd(x,na.rm=T)
  outliers<-if_else(abs(x-m)>3*s,T,F)
  return(outliers)
}
...
reglahampel<-function(x){
  ...
  m<-mean(x,na.rm=T)
  MADM<-1.4826*median(abs(x-m))
  outliers<-if_else(abs(x-m)>3*MADM,T,F)
  return(outliers)
}
...
df_outliers<-clean_iris %>%mutate(across(.cols=where(is.numeric),.fns=list(reglasigma=~reglasigma(.x),reglahampel=~reglahampel(.x),mreglaboxplot=~reglaboxplot(.x),reglapercentil=~reglapercentil(.x))))
```
...
resumen_outliers <- df_outliers %>%
  summarise(
    across(
      .cols = ends_with("reglasigma"),
      .fns = ~ sum(.x, na.rm = TRUE)
    ),
    across(
      .cols = ends_with("reglahampel"),
      .fns = ~ sum(.x, na.rm = TRUE)
    ),
    across(
      .cols = ends_with("reglaboxplot"),
      .fns = ~ sum(.x, na.rm = TRUE)
    ),
    across(
      .cols = ends_with("reglapercentil"),
      .fns = ~ sum(.x, na.rm = TRUE)
    )
  )
...
df_outliers_long <- df_outliers %>%
  # Seleccionamos la variable base y las columnas de outlier asociadas
  select(Petal.Length, starts_with("Petal.Length_")) %>%
  # Convertimos las columnas de las distintas reglas a formato largo
  pivot_longer(
    cols = starts_with("Petal.Length_"),
    names_to = "regla",
    values_to = "outlier"
  ) %>%
  # Opcional: limpiar nombres (quitar el prefijo "Petal.Length_")
  mutate(regla = sub("Petal.Length_", "", regla))

# Visualización
ggplot(df_outliers_long, aes(x = regla, y = Petal.Length, color = outlier)) +
  geom_point(position = "jitter", size = 2, alpha = 0.7) +
  scale_color_manual(values = c("TRUE" = "red", "FALSE" = "black", "NA" = "grey")) +
  labs(
    title = "Detección de outliers en Petal.Length según distintos métodos",
...
df_imputed<-df_outliers %>%  mutate(Petal.Length1=if_else(is.na(Petal.Length),mean(Petal.Length),Petal.Length))
...
df_imputed2<-df_outliers %>% group_by(Species) %>% 
  mutate(Petal.Length2=if_else(is.na(Petal.Length),mean(Petal.Length),Petal.Length))
...
df_outliers2 <- df_imputed2 %>%
  mutate(across(
    .cols = Petal.Length2,
    .fns = list(
      reglasigma = ~reglasigma(.x),
      reglahampel = ~reglahampel(.x),
      # CORRECCIÓN: Cambiamos 'mreglaboxplot' por 'reglaboxplot' para usar el nombre de la función
      reglaboxplot = ~reglaboxplot(.x), 
      reglapercentil = ~reglapercentil(.x)
    ),
    # Esto nombra las nuevas columnas con el nombre de la clave de la lista (reglasigma, reglahampel, etc.)
    .names = "{.fn}" 
  ))
...
# Solucion ejercicio 10a (CORREGIDO)
df_imputed2 <- df_na %>%
  # 1. Crear la variable de control 'imputado'
  mutate(imputado = is.na(Petal.Length)) %>%
  # 2. Imputar con el Método 1 (Ejemplo: Media Global)
  mutate(Petal.Length1 = coalesce(Petal.Length, mean(Petal.Length, na.rm = TRUE))) %>%
  # 3. Imputar con el Método 2 (Ejemplo: Mediana Global)
  mutate(Petal.Length2 = coalesce(Petal.Length, median(Petal.Length, na.rm = TRUE)))
...
df_outliers2 <- df_imputed2 %>%
  mutate(across(
    ...
    .fns = list(
      reglasigma = ~reglasigma(.x),
      reglahampel = ~reglahampel(.x),
      reglaboxplot = ~reglaboxplot(.x), # Clave corregida
      reglapercentil = ~reglapercentil(.x)
    ),
    .names = "{.fn}"
  ))

# Solucion ejercicio 10b (USANDO df_outliers2 CORRECTAMENTE)
df_outliers2 %>% 
  # Filtrar: Mantiene FALSE (no outlier) y NA (si es que los hay en la regla)
  filter(reglapercentil == FALSE | is.na(reglapercentil)) %>% 
  ggplot(aes(x = Petal.Length1, y = Petal.Length2, color = imputado)) +
  geom_point(alpha = 0.8) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "gray50") + 
  labs(
    title = "Comparación de dos Métodos de Imputación (Outliers por Percentil Eliminados)",
    subtitle = "Color indica si el dato original era NA",
    x = "Petal.Length Imputado con Método 1",
    y = "Petal.Length Imputado con Método 2",
    color = "¿Es Imputado?"
  ) +
