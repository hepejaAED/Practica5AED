...
df_imputed<-df_outliers %>%  mutate(Petal.Length1=ifelse(is.na(Petal.Length),mean(Petal.Length),Petal.Length))
...
df_imputed2<-df_outliers %>% group_by(Species) %>% 
  mutate(Petal.Length2=ifelse(is.na(Petal.Length),mean(Petal.Length),Petal.Length))
...
df_outliers2<-df_imputed2 %>%mutate(across(.cols=where(is.numeric),.fns=list(reglasigma=~reglasigma(.x),reglahampel=~reglahampel(.x),mreglaboxplot=~reglaboxplot(.x),reglapercentil=~reglapercentil(.x))))
...
df_outliers_long <- df_outliers %>%
  # Seleccionamos la variable base y las columnas de outlier asociadas
  select(Petal.Length2, starts_with("Petal.Length2")) %>%
  # Convertimos las columnas de las distintas reglas a formato largo
  pivot_longer(
    cols = starts_with("Petal.Length2"),
    names_to = "regla",
    values_to = "outlier"
  ) %>%
  # Opcional: limpiar nombres (quitar el prefijo "Petal.Length_")
  mutate(regla = sub("Petal.Length2", "", regla))

# Visualización
ggplot(df_outliers_long, aes(x = regla, y = Petal.Length2, color = outlier)) +
  geom_point(position = "jitter", size = 2, alpha = 0.7) +
  scale_color_manual(values = c("TRUE" = "red", "FALSE" = "black", "NA" = "grey")) +
  labs(
    title = "Detección de outliers en Petal.Length2 según distintos métodos",
    x = "Método de detección",
    y = "Petal.Length2",
    color = "Outlier"
  ) +
